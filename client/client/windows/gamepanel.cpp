...
void GamePanel::initButtonsGroup()
{
    ...
    connect(ui->btnGroup, &ButtonGroup::betPoint, this, [=](int bet){
        ...
        if(DataManager::getInstance()->getGameMode()==DataManager::Network){
            ...
            qDebug()<<"msg.roomName"<<msg.roomName;
...
void GamePanel::onPlayerStatusChanged(Player *player, GameControl::PlayerStatus status)
{
    ...
    case GameControl::Winning:
    {
        ...
        if(DataManager::getInstance()->isNetMode()){
            Message msg;
            msg.userName=DataManager::getInstance()->getUserName();
            msg.roomName=DataManager::getInstance()->getRoomName();
            msg.data1=QByteArray::number(m_gameCtl->getUserPlayer()->getScore());
            msg.reqCode=RequestCode::GameOver;
            DataManager::getInstance()->getCommun()->sendMessage(&msg);
        }
        break;
    }
...
void GamePanel::initGamePanel(QByteArray msg)
{
    ...
    for(const auto& it : lst){
        auto sub=it.split('-');
        order.insert(sub.at(1).toInt(),QPair(sub.at(0),sub.at(2).toUInt()));
...
void GamePanel::updatePlayerInfo(GamePanel::orderMap &info)
{
...
void GamePanel::notifyOtherPlayHand(Cards &cs)
{
    ...
    if(instance->getGameMode()==DataManager::Network){
        Message msg;
        ...
        instance->getCommun()->sendMessage(&msg);
    }
}
