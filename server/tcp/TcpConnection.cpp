...
int TcpConnection::processRead(void* arg)
{
    ...
    Debug("接收到请求数据: %s", conn->m_readBuf->data());
    if (count > 0)
    {
        //解析数据
        Debug("//解析数据");
...
void TcpConnection::prepareSecretKey(){
    ...
    msg.data1=pubkey;
    RsaCrypto rsa;
    rsa.parseStringToKey(redis.rsaSecKey("PrivateKey"),RsaCrypto::KeyType::PrivateKey);
    msg.data2=rsa.sign(pubkey);

    Codec codec(&msg);
    std::string data=codec.encodeMsg();
...
void TcpConnection::addWriteTask(std::string data)
{
    ...
    //通过事件发送
            ...
            conn->m_channel->setCurrentEvent(FDEvent::WriteEvent);
            // 2. 修改dispatcher检测的集合 -- 添加任务节点
            conn->m_evLoop->addTask(conn->m_channel, ElemType::MODIFY);
#else
    //直接发送
    m_writeBuf->sendData(m_channel->getSocket());
    Debug("m_writeBuf->sendData(m_channel->getSocket());");
#endif
}

void TcpConnection::addDeleteTask()
{
    Debug("断开客户端连接,connName=%s",m_name.data());
